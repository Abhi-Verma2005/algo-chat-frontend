...
import { Trophy, Target, Code, CheckCircle, ExternalLink, TrendingUp } from 'lucide-react';
...
export default function DSAProgressDashboard({ data = SAMPLE_DATA }) {
  const { user, overview, tagProgress, recentActivity } = data;
  ...
  return (
    <div className="max-w-md mx-auto bg-black border border-gray-800 rounded-2xl p-4 space-y-4 shadow-2xl">
      ...
      <div className="text-center space-y-2">
        ...
        <div>
          ...
          <div className="inline-block bg-gray-800 px-2 py-1 rounded-full border border-gray-700 text-xs font-bold mt-1 text-green-400">
            🌱 {user.rank?.replace('_', ' ').toUpperCase() || 'NOVICE 1'}
          </div>
        </div>
      </div>
      ...
      {/* Tag Progress */}
      {tagProgress && tagProgress.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Target className="w-4 h-4 text-blue-500" />
            <h3 className="text-sm font-bold text-white">Topic Progress</h3>
          </div>
          <div className="space-y-2">
            {tagProgress.slice(0, 3).map((tag, index) => (
              <div key={index} className="flex items-center justify-between text-xs">
                <span className="text-gray-300">{tag.tagName}</span>
                <span className="text-white">
                  {tag.solvedProblems}/{tag.totalProblems}
                </span>
              </div>
            ))}
          </div>
